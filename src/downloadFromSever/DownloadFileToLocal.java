...
public class DownloadFileToLocal {
	public static boolean downloadFile(String id) throws SQLException, ClassNotFoundException, IOException {
		...
		String sql = "select sever_Address,username_Sever,password_Sever,port_Sever,remote_DirSever,local_Dir"
				+ ",file_Name_Config,extension,mail_Recieved from myconfig where id=" + id;
		...
		while (rs.next()) {
			...
			success = scp.SyncTreeDownload(remoteDir, localDir, 6, false);// (5)
	...
	public static void logs(String id) throws ClassNotFoundException, SQLException, IOException {
		...
		String status = "ER"; // (7)
		String selectfilename = "select file_name from log where file_name=?"; // (7)
		PreparedStatement pre = conn.prepareStatement(sql);
		ResultSet rs = null;
		rs = pre.executeQuery();
		rs.beforeFirst();
